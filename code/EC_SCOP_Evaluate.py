...
class EC_SCOP_Evaluate:
	...
	def create_Xy(self):
		...
		self.domains_labels[["x_"+str(i) for i in range(self.get_emb_num_dim())]] = pd.DataFrame(self.domains_labels.vector.tolist(), index=self.domains_labels.index)
	...
	def read_id_ec_scop(self):
		...
		self.domains_labels = pd.read_csv(join(self.data_path, self.id_ec_scop_file), sep="\t", header=0)#, dtype={"interpro_id":str,"ECs":str,"SCOPs":str}
	...
	def get_scop_class(self, domains, remove_multilabeled):
		...
		:param domains: dataframe
		:param remove_multilabeled: True or False (remove or not doamins with more than on SCOP class)
		:return: None
		"""
	...
	def get_principalEC(self, domains, remove_multilabeled):
		"""
		Converting specific EC code to the principal EC that belongs to
		...
		:param domains: dataframe
		:param remove_multilabeled: True or False (remove or not domains with more than one EC numbers)
		:return: None
		"""
	...
	def pred_by_center(self, cluster_centers, labels):
		...
		:param cluster_centers: coordinates of each cluster center
		:param labels: predicted label of each point (cluster index)
	...
	def cluster_Xy(self, are_dim_reduced, cluster_algo):
		...
		if cluster_algo == "k-means":
			...
			k_means = KMeans(n_clusters=num_uniq_classes, random_state=self.random_state)

			# k_means_fitted = k_means.fit(self.domains_labels[["x_" + str(i) for i in range(self.get_emb_num_dim())]])
			# y_pred = self.pred_by_center(k_means_fitted.cluster_centers_, k_means_fitted.labels_)
			#print(k_means_fitted.cluster_centers_.shape)
			#print(k_means_fitted.labels_.shape)
			# classified_data = k_means.labels_
			# y_pred = classified_data
			if are_dim_reduced:
				y_pred = KMeans(n_clusters=num_uniq_classes, random_state=self.random_state).fit_predict(self.X_low)
			else:
				y_pred = KMeans(n_clusters=num_uniq_classes, random_state=self.random_state).fit_predict(self.get_x().values)
	...
	def purity_score(self, y_true, y_pred):
		#Compute purity as in https://nlp.stanford.edu/IR-book/html/htmledition/evaluation-of-clustering-1.html
	...
	def run_NN(self,are_dim_reduced):
		...
		for nn_num in [2, 5, 20, 40]:
			...
			print("max test acc: {}, min test acc: {}".format(max(test_acc_models[nn_num]), min(test_acc_models[nn_num])))
	...
	def run_GMM(self, are_dim_reduced):
		...
		for model_name in ['spherical', 'diag', 'tied', 'full']:
			...
			print("max train acc: {}, min test acc: {}".format(max(train_acc_models[model_name]), min(train_acc_models[model_name])))
			print("avg train acc: {}".format(sum(train_acc_models[model_name])/float(len(train_acc_models[model_name]))))
			print("max test acc: {}, min test acc: {}".format(max(test_acc_models[model_name]), min(test_acc_models[model_name])))
	...
	def run_tsne(self, perplexity, low_dim_size):
		print("Run t-SNE")
		tsne = TSNE(perplexity=perplexity, n_components=low_dim_size, init='pca', n_iter=2500, random_state=self.random_state)
	...
	def plot_low(self, low_method, low_dim_size):
		...
		if self.use_ec:
			ax.scatter(X_low[:, 0], X_low[:, 1], c=[int(label)-1 for label in self.domains_labels["ECs"].values], cmap=cmap, norm=norm, edgecolor='k')
			# Credits: https://stackoverflow.com/questions/31303912/matplotlib-pyplot-scatterplot-legend-from-color-dictionary
			markers = [plt.Line2D([0, 0], [0, 0], color=cmap(norm(ec_num-1)), marker='o', linestyle='') for ec_num, _ in self.ec_num2name.items()]
			plt.legend(markers, [ec_name for _, ec_name in self.ec_num2name.items()], numpoints=1, loc=9, bbox_to_anchor=(0.5, -0.1), ncol=2)
		else:
			ax.scatter(X_low[:,0], X_low[:,1], c=[int(label)-1 for label in self.domains_labels["SCOPs"].values], cmap=cmap, norm=norm,edgecolor='k')
			markers = [plt.Line2D([0, 0], [0, 0], color=cmap(norm(scop_num-1)), marker='o',linestyle='') for scop_num, _ in self.scop_num2name.items()]
			plt.legend(markers, [scop_name for _,scop_name in self.scop_num2name.items()], numpoints=1, loc=9, bbox_to_anchor=(0.5, -0.1), ncol=2)
		# plt.show()
	...
	def convert_dom2Xy(self, model_file, is_model_bin):
		...
		self.domains_labels[["x_"+str(i) for i in range(self.get_emb_num_dim())]] = self.domains_labels[["x_"+str(i) for i in range(self.get_emb_num_dim())]].astype(float)
	...
	#todo: remove if clustering is not in use

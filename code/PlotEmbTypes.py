...
class PlotEmbTypes:
	...
	def __init__(self,dom_types_file,data_path,model_file,is_model_binary,dim_size):
		...
		#following the release notes, color the domains type:
		#self.types2colors = {"Active_site": "purple", "Binding_site": "purple", "Conserved_site": "purple", "PTM": "purple", "Repeat": "orange", "Homologous_superfamily": "blue", "Family": "red", "Domain": "green"}
		self.types2colors = {"PMP-22/EMP/MP20/Claudin superfamily":"red","small GTPase superfamily":"blue","Kinase-pyrophosphorylase":"green","Exonuclease, RNase T/DNA polymerase III":"cyan","SH2 domain":"magenta"}
	...
	def map_superfamily2color(self):
		...
		for interpro_id in self.vocab:
			...
			superfamily_type = self.domain_types.loc[self.domain_types['interpro_id'] == interpro_id,'superfamily'].iloc[0]
			assert superfamily_type in self.types2colors, "AssertionError: domain superfamily {} not found".format(superfamily_type)
			self.vocab_colors.append(self.types2colors[superfamily_type])

	def inter2color(self):
		print("Map interpro ids to colors")
		for interpro_id in self.vocab:
			# assert interpro_id in self.domain_types['ENTRY_AC'].values
	...
	def plot_low(self, low_method,title_msg):
		...
		if self.dim_size == 2:
			...
			markers = [plt.Line2D([0, 0], [0, 0], color=color, marker='o', linestyle='') for color in self.types2colors.values()]
			plt.legend(markers, [key.replace("_", " ") for key in self.types2colors.keys()], numpoints=1, loc=9, bbox_to_anchor=(0.5, -0.1), ncol=2)
		else:
